...
async function getPollImage(pollId, status) {
    ...
    // Icon
    await loadImage(
        `${__dirname}/../assets/images/poll-icon.png`
    ).then((image) => {
    ...
    for (const option in options) {
        ...
        // Poll Ring
        await loadImage(
            `${__dirname}/../assets/images/${pollRing}.png`
        ).then((image) => {
...
async function updatePolls(client) {
    ...
    for (const poll in polls) {
        ...
        const pollStatus = currentPoll.expiryTime != 0 && expiryTime < Date.now() ? PollStatus.Closed : PollStatus.Open
